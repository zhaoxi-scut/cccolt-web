...
在开发完一个库的时候，需要将其打包给他人使用（注意不是打包让他人源码编译，而是直接拿来用），我们需要提供一个包含这个库里面所有待导出目标信息的 `*.cmake` 文件，称为导出配置文件。导出目标是个目标，是虚拟的概念，而导出配置文件是个实际存在的 `*.cmake` 文件，该文件是在 CMake 的生成阶段由导出目标生成的。一般情况下，我们在 Debian 系 Linux 发行版上利用 APT 包管理工具的时候，在执行以下命令时

```bash
sudo apt install xxx-dev
```

如果该库是由 CMake 构建的，那么安装的内容就一定包含该导出目标对应的 `*.cmake` 文件，不过，这一文件我们并不会直接使用，而是间接的通过使用 `XxxConfig.cmake` 的方式使用该导出目标对应的 `*.cmake` 文件。我们先回到最基本的，如何添加导出配置。
...
我们就以 OpenCV 在执行完安装命令之后生成的导出配置文件

1. `OpenCVModules.cmake`
2. `OpenCVModules-release.cmake`
...
3. **创建导入目标，并初步设置属性**
   ...
   可能有读者会注意到，这里并没有为导入目标设置 `IMPORTED_LOCATION` 属性。没错，所以说这里是<font color="red">初步</font>设置属性。

4. 包含 `OpenCVModules-<config>.cmake` 文件，在此文件中<font color="red">进一步</font>设置属性。如果该项目是
...
在[【05】目标构建](05.md)一文中介绍了 `target_include_directories` 和 `target_link_libraries` 的用法，并指出尽量不要使用 `include_directories` 和 `link_libraries` 两种，下面将根据本文导出配置的内容介绍为何要使用 `target_xxx`。
...
这便是使用 `target_xxx` 命令的作用之一，可以将链接、包含关系绑定至目标，在导出并安装时，会链式的将这些链接、包含关系继续绑定在该目标上，而使用 `include_directories` 和 `link_libraries` 则没有这些效果。当然，在不需要进行导出安装的项目中，完全可以使用这些不带 `target` 前缀的这些命令。不过这些命令会作为全局包含的形式给其他目标使用，有一定的 “污染” 嫌疑。
...
以下展示了 OpenCV 4.8.0 的用于 `find_package` 的 Config 文件
